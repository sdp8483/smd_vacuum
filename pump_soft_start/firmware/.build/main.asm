...
;	-----------------------------------------
_main:
;	main.c: 31: PAC &= ~(1 << INPUT_PIN);         /* Set input as input (all pins are input by default) */
	set0.io	__pac, #5
;	main.c: 32: PADIER |= (1 << INPUT_PIN);       /* input as digital input */
	set1.io	__padier, #5
;	main.c: 33: PAPH |= (1 << INPUT_PIN);         /* Enable pull-up resistor on input */
	set1.io	__paph, #5
;	main.c: 34: PAC |= (1 << PWM_OUTPUT_PIN);     /* Set PWM_OUPUT_PIN as output (all pins are input by default) */
	set1.io	__pac, #4
;	main.c: 37: PWMG1CUBL = PWM_MAX << 5;         /* Set the PWM upper bound (lower 3 bits) */
	mov	a, #0xe0
	mov.io	__pwmg1cubl, a
;	main.c: 38: PWMG1CUBH = PWM_MAX >> 3;         /* (upper 5 bits) */
	mov	a, #0x1f
	mov.io	__pwmg1cubh, a
;	main.c: 39: PWMG1DTL = 0x00;                  /* Clear the PWM duty value */
	mov	a, #0x00
	mov.io	__pwmg1dtl, a
;	main.c: 40: PWMG1DTH = 0x00;
	mov	a, #0x00
	mov.io	__pwmg1dth, a
;	main.c: 41: PWMG1C = (uint8_t)(PWMG1C_ENABLE | PWMG1C_OUT_PA4 | PWMG1C_CLK_IHRC);
	mov	a, #0x87
	mov.io	__pwmg1c, a
;	main.c: 42: PWMG1S = 0x00;                    /* No pre-scaler */
	mov	a, #0x00
	mov.io	__pwmg1s, a
;	main.c: 46: while (1) {
00109$:
;	main.c: 48: if (isInputActive()) {
	mov.io	a, __pa
	and	a, #0x20
	ceqsn	a, #0x00
	goto	00106$
;	main.c: 51: for (pwmValue = PWM_MIN; pwmValue < PWM_MAX; pwmValue += PWM_INC_VALUE) {
	mov	a, #0x80
	mov	_main_sloc0_1_0+0, a
	clear	_main_sloc0_1_0+1
00111$:
;	main.c: 52: PWMG1DTL = pwmValue << 5;     /* Set PWM duty value (lower 3 bits) */
	mov	a, _main_sloc0_1_0+0
	swap	a
	and	a, #0xf0
	sl	a
	mov.io	__pwmg1dtl, a
;	main.c: 53: PWMG1DTH = pwmValue >> 3;     /* (upper 8 bits) */
	...
	src	a
	mov.io	__pwmg1dth, a
;	main.c: 54: _delay_ms(PWM_RAMP_DELAY_ms);
	mov	a, #0xe6
...
;	main.c: 51: for (pwmValue = PWM_MIN; pwmValue < PWM_MAX; pwmValue += PWM_INC_VALUE) {
	...
	t0sn.io	f, c
	goto	00111$
;	main.c: 58: PWMG1DTL = PWM_MAX << 5;     /* Set PWM duty value (lower 3 bits) */
	mov	a, #0xe0
	mov.io	__pwmg1dtl, a
;	main.c: 59: PWMG1DTH = PWM_MAX >> 3;     /* (upper 8 bits) */
	mov	a, #0x1f
	mov.io	__pwmg1dth, a
;	main.c: 60: while(isInputActive()) {
00102$:
	mov.io	a, __pa
	and	a, #0x20
	ceqsn	a, #0x00
	goto	00109$
;	main.c: 61: _delay_ms(1000);
	mov	a, #0x84
	...
	goto	00102$
00106$:
;	main.c: 65: PWMG1DTL = 0;                 /* Set PWM duty value (lower 3 bits) */
	mov	a, #0x00
	mov.io	__pwmg1dtl, a
;	main.c: 66: PWMG1DTH = 0;                 /* (upper 8 bits) */
	mov	a, #0x00
	mov.io	__pwmg1dth, a
;	main.c: 67: _delay_ms(50);                /* short delay for debounce */
	mov	a, #0x68
	mov	__delay_loop_16_PARM_1+0, a
	mov	a, #0x18
	mov	__delay_loop_16_PARM_1+1, a
...
;	main.c: 70: }
	ret
;	main.c: 73: unsigned char _sdcc_external_startup(void) {
;	-----------------------------------------
;	 function _sdcc_external_startup
;	-----------------------------------------
__sdcc_external_startup:
;	main.c: 78: AUTO_INIT_SYSCLOCK();
	mov	a, #0x1c
	mov.io	__clkmd, a
;	main.c: 83: AUTO_CALIBRATE_SYSCLOCK(TARGET_VDD_MV);
	and	a, #'R'                       
	and	a, #'C'                       
	and	a, #(1)            
	and	a, #((1000000))     
	...
	and	a, #((5000)>>8)  
	and	a, #(0x0b)             
;	main.c: 85: return 0;   // Return 0 to inform SDCC to continue with normal initialization.
;	main.c: 86: }
	ret	#0x00
	.area CODE
	.area CONST
